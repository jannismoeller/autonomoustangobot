...
namespace octomapcustomization{
    ...
    NavigationOcTree::NavigationOcTree(double resolution)
        ...
        __android_log_print(ANDROID_LOG_INFO, "OCTREE", "keyrays.size(): %u", (unsigned int)keyrays.size());
    }
    ...
    /**
     * Source: https://stackoverflow.com/a/41637260
     * @return 
     */
    double invsqrtQuakeDouble( double number )
    {
        double y = number;
        double x2 = y * 0.5;
...
//	y  = y * ( threehalfs - ( x2 * y * y ) );   // 2nd iteration, this can be removed
    ...
    void NavigationOcTree::computeDiscreteUpdate(const Pointcloud &scan, 
        ...
        // origin + deltaV3.norm * maxrange as relevant point for dropping rays instead of the endpoints
        octomath::Vector3 v3;
...
//#ifdef _OPENMP
////        omp_set_num_threads((int)this->keyrays.size());
//#pragma omp parallel for num_threads((int)this->keyrays.size())
//#endif
        for (int i = 0; i < (int) scan.size(); ++i) {
//            distance_sq = (v3 = (scan[i] - origin)).norm_sq();
//            if(distance_sq > maxrange_sq){
////                v3 *= ((float)maxrange / sqrt((float)distance_sq));
////                v3 *= (maxrange * invsqrtQuakeDouble(distance_sq));
//                v3 *= ((float)maxrange * q_rsqrt((float)distance_sq));
//                k = this->coordToKey(origin + v3);
//                outOfRange++;
//            } else {
                k = this->coordToKey(scan[i]);
//            }
//#ifdef _OPENMP
////#pragma omp critical (discretePC_push_back)
//#pragma omp critical
//#endif
            {
        ...
        __android_log_print(ANDROID_LOG_INFO, "OCTREE",
                            "Insertion has been descretized by %d of %d points",
                            outOfRange, total);
    ...
    bool NavigationOcTree::isInRange(point3d botPosition, float botRadius, point3d cellCoordinate, float cellSize) {
    ...
    bool NavigationOcTree::testCollisionWithCylinder(point3d pos,
        ...
        // X / 1. Coordinate is pointing FORWARD
        // Y / 2. Coordinate is pointing LEFT
        // Z / 3. Coordinate is pointing UP
        NavigationOcTree::leaf_bbx_iterator it = begin_leafs_bbx(point3d(pos.x() - radius, pos.y() - radius, pos.z() - 0),
    ...
    void NavigationOcTree::addNodeToBufferRecursive(jfloat **pFB, int *vertexCount, int bufferMaxVerts,
            ...
            for (unsigned int i = 0; i < 8 && *vertexCount < bufferMaxVerts; ++i) {
                if (this->nodeChildExists(node, i)) {
                    addNodeToBufferRecursive(pFB, vertexCount, bufferMaxVerts,
                                             ...
                                             halfNodeSize, nodePos + (DIRECTIONS[i] * quarterNodeSize));
                }
            }
        } else {
            *((*pFB)++) = nodePos.x();
...
//        __android_log_print(ANDROID_LOG_INFO, "OCTREE", "Entering the recursion");
    ...
    int NavigationOcTree::fillBufferWithNodePointsViaLeafIterator(jfloat *pFB,
        ...
        for (; it != itEnd && vertCount < bufferMaxVertsCount; it++) {
            ...
            if (this->isNodeOccupied(node)) {
                ...
                pFB[vertCount * 3] = nodePos.x();
                pFB[vertCount * 3 + 1] = nodePos.y();
                pFB[vertCount * 3 + 2] = nodePos.z();
                // for visualization of navigation information the 4th float of the vert (in XYZW Format) 
                // is set to 1 if the node is navigable by the bot
                assert(node->navInfo);
                pFB[vertCount * 3 + 3] = (node->navInfo->neighborCount != 0) ? 1.0f : 0.0f;
    ...
    void NavigationOcTree::searchFromCurrent(array<NavigationNode *, stackSize> *stackToCurrentNode,
        ...
        if((*stackToCurrentNode)[parentDepth] == NULL)
            __android_log_print(ANDROID_LOG_INFO, "OCTREE", "debug_searchFromCurrent_count: %d\n"
                                        "failed with parentDepth: %d\n"
                                        "and *currentDepth: %d",
                                debug_searchFromCurrent_count, parentDepth, *currentDepth);
...
//                *currentDepth = ++parentDepth;
    ...
    void NavigationOcTree::getUpperNeighbor(array<NavigationNode *, stackSize> *stackToCurrentNode,
        ...
        // TODO: check new key for boundaries
...
//        __android_log_print(ANDROID_LOG_INFO, "OCTREE COLLISION", "trying to access parentDepth: %i", parentDepth);
        while (this->nodeHasChildren((*stackToCurrentNode)[parentDepth])) {
...
//            __android_log_print(ANDROID_LOG_INFO, "OCTREE COLLISION", "trying to access parentDepth: %i", parentDepth + 1);
            (*stackToCurrentNode)[parentDepth + 1] = this->getNodeChild(
                    (*stackToCurrentNode)[parentDepth], childIndex);
            parentDepth++;
        }
...
//            __android_log_print(ANDROID_LOG_INFO, "OCTREE COLLISION", "start node is occupied");
            return 1;
        }
        ...
        double nextPos;
        double currentNodeKeyZPos;
        double currentNodeSize;
        uint16_t currentKeyZ;
        int i = 0;
        while ((nextPos = (currentNodeKeyZPos = this->keyToCoord((currentKeyZ = (*currentNodeKey)[2]))) +
                          (currentNodeSize = this->getNodeSize(
                                  static_cast<unsigned int>(*currentDepth))) - smallestSizeHalf) <=
               maxZCoord) {
...
//                                    "OCTREE COLLISION",
//                                    "still running because: %f is below %f\ncurrentKeyZ: %u, currentNodeKeyZPos: %f\ncurrentNodeSize: %f\nsmallestSizeHalf: %f",
//                                    nextPos,
//                                    maxZCoord,
//                                    currentKeyZ,
//                                    currentNodeKeyZPos,
...
//                                    smallestSizeHalf);
    ...
    void NavigationOcTree::handleNodeFindNodesWithFreeSpaceAbove(
        ...
        // Set the trailing z key bits that are below the depth of this Node to zero 
        // to get the correct upper Neighbor
//        currentNodeKey[2] &= 0xFFFF << (getTreeDepth() - currentDepth);

//        __android_log_print(ANDROID_LOG_INFO, "OCTREE", "handling node at depth: %d, "
//                                    "X-key for neighborsearch: %04x "
//                                    "Y-key for neighborsearch: %04x "
//                                    "Z-key for neighborsearch: %04x", 
//                            currentDepth, currentNodeKey[0], currentNodeKey[1], currentNodeKey[2]);

//        __android_log_print(ANDROID_LOG_INFO, "OCTREE", "nodeStackCopy[currentDepth]: %ld" , reinterpret_cast<long>(nodeStackCopy[currentDepth]));
        NavigationNode * node = nodeStackCopy[currentDepth];
//        __android_log_print(ANDROID_LOG_INFO, "OCTREE", "Going into getUpperNeighbor");
        getUpperNeighbor(&nodeStackCopy, &currentNodeKey, &currentDepth);
//        __android_log_print(ANDROID_LOG_INFO, "OCTREE", "Coming out of getUpperNeighbor");

//        double botHeight = 0.25;
//        if(raySteppingUp(currentNodeKey, (*node).navInfo.zCoord + botHeight))
...
//        } else if(!isNodeOccupied(nodeNeighbor)){
        } else {
            int rayResult = raySteppingUp(&nodeStackCopy, &currentNodeKey, &currentDepth,
                                          maxZCoord);
            if (rayResult == 0) {
//            __android_log_print(ANDROID_LOG_INFO, "OCTREE", "setting Node to navigatable");
    ...
    void NavigationOcTree::handleNodeFindNeighbors(
        ...
        for (uint8_t i = 0; i < 8; ++i) {
                    ...
                    if (neighbor != NULL) {
                        if (isNodeOccupied(neighbor)){
                            if(neighbor->navInfo->hasFreeSpaceAbove &&
                                    abs(node->navInfo->zCoord - neighbor->navInfo->zCoord) <= stepHeight) {
                                ...
                                neighbor->setNeighbor(NavigationNode::getOppositeNeighborIdx(i), node);
                            } else {
            ...
            if(node->navInfo->neighbors[i] != NULL){
                ...
                if(node != node->navInfo->neighbors[i]->navInfo->neighbors[NavigationNode::getOppositeNeighborIdx(i)])
                    __android_log_print(ANDROID_LOG_ERROR, "OCTREE", "ASSERTION FAILED: debug_connected_neighbar_newly %d", debug_connected_neighbar_newly);
                assert(node == node->navInfo->neighbors[i]->navInfo->neighbors[NavigationNode::getOppositeNeighborIdx(i)]);
            }
        }
    ...
    void NavigationOcTree::handleNodeReduceNeighbors(
        ...
        if(nullNeighborCount >= node->navInfo->removedNeighborCount + nullNeighborsNecessaryForReduction) {
            ...
            for (int i = 0; i < 8; ++i) {
                ...
                if (neighbor != NULL) {
                    ...
                    assert(neighbor->navInfo->neighbors[NavigationNode::getOppositeNeighborIdx(i)] == node);
                    ...
                    assert(neighbor->navInfo->removedNeighborCount == debug_neighbor_removedneighborcount + 1);
     ...
     */
    void NavigationOcTree::iterateLeafsRecursive(array<NavigationNode *, stackSize> *stackToCurrentNode,
                                                 OcTreeKey currentNodeKey, int currentDepth,
            ...
            for (unsigned int i = 0; i < 8; ++i) {
                if (this->nodeChildExists(node, i)) {
                    ...
                    iterateLeafsRecursive(stackToCurrentNode, childNodeKey, currentDepth,
                                          currentShiftsFromBottom, handleNode, stepHeight, botHeight);
                }
            }
        } else {
            (this->*handleNode)(stackToCurrentNode, currentNodeKey, currentDepth, stepHeight, botHeight);
        }
    }
     ...
     */
    void NavigationOcTree::iterateBoundingBoxRecursive(array<NavigationNode *, stackSize> *stackToCurrentNode,
        ...
        if (this->nodeHasChildren(node)) {
            ...
            if(keyAllComponentsBiggerOrEqual(maxKeyNode, minKeyBBX) &&
               keyAllComponentsSmallerOrEqual(minKeyNode, maxKeyBBX)){ // Node is overlapping
                ...
                if (keyAllComponentsBiggerOrEqual(minKeyNode, minKeyBBX) &&
                    keyAllComponentsSmallerOrEqual(maxKeyNode, maxKeyBBX)) {// Node completely in BBX
                    ...
                    for (unsigned int i = 0; i < 8; ++i) {
                        if (this->nodeChildExists(node, i)) {
                            ...
                            iterateBoundingBoxRecursive(stackToCurrentNode, currentDepth, currentShiftsFromBottom,
                                                        minKeyBBX, maxKeyBBX, childMinKey, childMaxKey, handleNode,
                                                        stepHeight, botHeight);
                        }
                    }
                }
            ...
            (this->*handleNode)(stackToCurrentNode, minKeyNode, currentDepth, stepHeight, botHeight);
        }
    }
    ...
    void NavigationOcTree::iterateBoundingBox(OcTreeKey minKeyBBX, OcTreeKey maxKeyBBX,
        ...
        for (; currentDepth < leadingZeros && this->nodeHasChildren(stackToParent[currentDepth]); ++currentDepth) {
            uint8_t childIndex = computeChildIdx(minKeyNode, this->getTreeDepth() - 1 - currentDepth);
            if(this->nodeChildExists(stackToParent[currentDepth], childIndex))
                stackToParent[currentDepth + 1] = this->getNodeChild(stackToParent[currentDepth], childIndex);
            else {
                // Handle early out for empty BBX
            }
        }

        iterateBoundingBoxRecursive(&stackToParent, currentDepth, this->getTreeDepth() - currentDepth,
     ...
     */
    NavigationNode* NavigationOcTree::setNodeValue(const OcTreeKey& key, float log_odds_value, bool lazy_eval, float zCoord) {
        ...
        return setNodeValueRecurs(this->root, createdRoot, key, 0, log_odds_value, lazy_eval, zCoord);
    }
     ...
     */
    NavigationNode* NavigationOcTree::setNodeValueRecurs(NavigationNode* node, bool node_just_created, const OcTreeKey& key,
        ...
        if (depth < this->tree_depth) {
            ...
            if (lazy_eval)
                return setNodeValueRecurs(this->getNodeChild(node, pos), created_node, key, depth+1, log_odds_value, lazy_eval, zCoord);
            else {
                NavigationNode* retval = setNodeValueRecurs(this->getNodeChild(node, pos), created_node, key, depth+1, log_odds_value, lazy_eval, zCoord);
...
// end change
     ...
     */
    NavigationNode * NavigationOcTree::updateNode(const OcTreeKey &key, bool occupied, bool lazy_eval, float zCoord) {
     ...
     * didn't make "updateNodeRecurs" a virtual method
     */
    NavigationNode * NavigationOcTree::updateNode(const OcTreeKey &key, float log_odds_update, bool lazy_eval, float zCoord) {
        ...
        return updateNodeRecurs(this->root, createdRoot, key, 0, log_odds_update, lazy_eval, zCoord);
    }
     ...
     * of the NavigationNodes are marked by comments
     */
    NavigationNode* NavigationOcTree::updateNodeRecurs(NavigationNode* node, bool node_just_created, const OcTreeKey& key,
        ...
        if (depth < this->tree_depth) {
            ...
            if (lazy_eval)
                return updateNodeRecurs(this->getNodeChild(node, pos), created_node, key, depth+1, log_odds_update, lazy_eval, zCoord);
            else {
                NavigationNode* retval = updateNodeRecurs(this->getNodeChild(node, pos), created_node, key, depth+1, log_odds_update, lazy_eval, zCoord);
                // prune node if possible, otherwise set own probability
                // note: combining both did not lead to a speedup!
                if (this->pruneNode(node)){
                    // return pointer to current parent (pruned), the just updated node no longer exists
...
//                            debug_total_leafnode_count);
    ...
    void NavigationOcTree::updateAdjacencyNet(float botMaxStepHeight, OcTreeKey changeBBXMin,
                                              OcTreeKey changeBBXMax) {
        __android_log_print(ANDROID_LOG_INFO, "OCTREE", "Going into iterateBoundingBox with min: %04x, %04x, %04x max: %04x, %04x, %04x",
                            changeBBXMin[0], changeBBXMin[1], changeBBXMin[2],
                            changeBBXMax[0], changeBBXMax[1], changeBBXMax[2]);
        this->iterateBoundingBox(changeBBXMin, changeBBXMax, &NavigationOcTree::handleNodeFindNeighbors, botMaxStepHeight, 0.0f);
    }
    ...
    void NavigationOcTree::reduceAdjacencyNet(int reductionIterations, OcTreeKey changeBBXMin,
        ...
        for (int i = 0; i < reductionIterations; ++i) {
            __android_log_print(ANDROID_LOG_DEBUG, "OCTREE", "reduceAdjacencyNet: iteration: %d", i);
            this->iterateBoundingBox(changeBBXMin, changeBBXMax,
                                     &NavigationOcTree::handleNodeResetCellularFields, 0.0f, 0.0f);
            this->iterateBoundingBox(changeBBXMin, changeBBXMax, &NavigationOcTree::handleNodeReduceNeighbors, 0.0f, 0.0f);
        }
    }
    ...
    bool NavigationOcTree::updateNavigabilityAndAdjacencyNet(float botRadius, float botHeight,
        ...
        // Step 1: 
        __android_log_print(ANDROID_LOG_INFO, "OCTREE", "updateNodesWithFreeSpaceAbove begin with height: %f", botHeight);
        ...
        this->iterateBoundingBox(bbxMinAdjacencyNet, bbxMaxAdjacencyNet, &NavigationOcTree::handleNodeResetFrontier, 0.0f, 0.0f);

        // Step 3:
        __android_log_print(ANDROID_LOG_INFO, "OCTREE", "reduceAdjacencyNet begin of %d reductionIterations", reductionIterations);
    ...
    bool NavigationOcTree::findNextNavigableNode(const point3d pos, const float range,
        ...
        for (; it != itEnd; it++) {
            ...
            if (isNodeOccupied(tempNode) && tempNode->getNavigationInformation()->neighborCount > 0) {
    ...
    AStarPath* NavigationOcTree::findNextPath(const point3d botPosition, const point3d botDirection,
        ...
        vector<NavigationOcTree::NodeCluster*> *clusterList = clusterNavigableFrontierNodes(clusterMaxRange);
        ...
        vector<PathToCluster *> *pathList = calculatePathsToClusters(clusterList, botPosition, botNodeSearchRange);
        ...
        for(PathToCluster *path : *pathList){
            if(path->path == NULL) {
                ...
                path->clusterNodeCountNormalized = (double)path->cluster->getNodeCount() / largestCluster;
        ...
        if(bestPath->path == NULL){
            __android_log_print(ANDROID_LOG_DEBUG, "OCTREE", "seems like %lu paths were NULL", pathList->size());
        } else {
            __android_log_print(ANDROID_LOG_DEBUG, "OCTREE", "seems like bestPath != NULL, length: %f number of waypoints: %lu\n"
